...
const SUITE = 'supabase/tests/child_account_authorization.sql'
...
if (!existsSync(SUITE)) fail(`Suite not found: ${SUITE}`)
...
try {
  const output = execFileSync('psql', [LOCAL_DB_URL, '-v', 'ON_ERROR_STOP=1', '-f', SUITE], {
    encoding: 'utf8',
    stdio: ['ignore', 'pipe', 'pipe'],
  })
  const failures = output.split('\n').filter((line) => line.includes('ASSERTION FAILED'))
  if (failures.length) fail(`Authorization assertions failed:\n${failures.join('\n')}`)
  console.log('Child account authorization matrix: all assertions passed.')
} catch (error) {
  if (error.code === 'ENOENT') {
    fail('psql not found on PATH. Install the PostgreSQL client, or run:\n  npx supabase db reset\n  docker exec -i supabase_db_Rodinka psql -U postgres -v ON_ERROR_STOP=1 < ' + SUITE)
  }
  const detail = `${error.stdout ?? ''}${error.stderr ?? ''}`.trim()
  fail(`Authorization suite failed:\n${detail || error.message}`)
}
