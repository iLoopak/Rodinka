...
export function ReminderCenter() {
  ...
  return <div className="reminder-center">
    ...
    {tab === 'history' && (history.length === 0 ? <div className="reminder-empty"><h2>{t.reminders.historyEmpty}</h2></div> : <ul className="reminder-list reminder-history">{history.map((item) => <ReminderCard key={item.id} item={item} onOpen={() => open(item)} onRead={() => markRead(item.id)} onDismiss={() => dismiss(item.id)} />)}</ul>)}
