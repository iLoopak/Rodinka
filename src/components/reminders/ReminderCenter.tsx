...
function PushSettings({ preferences, saving, onChange }: { preferences: NotificationPreferences; saving: boolean; onChange: (next: NotificationPreferences) => Promise<void> }) {
  ...
  return <div className="push-settings">
    ...
    {showIntro && <div className="push-consent" role="dialog" aria-label={t.reminders.consentLabel}><strong>{t.reminders.consentTitle}</strong><p>{t.reminders.consentBody}</p><div className="modal-actions"><button type="button" onClick={enable} disabled={push.busy}>{t.reminders.allow}</button><button type="button" className="btn-secondary" onClick={() => setShowIntro(false)} disabled={push.busy}>{t.reminders.notNow}</button></div></div>}
