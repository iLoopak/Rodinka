...
describe('FilterDisclosure', () => {
  ...
  it('exposes active filters and resets them without opening the panel', () => {
    const onClear = vi.fn()
    render(createElement(FilterDisclosure, { id: 'filters', open: false, onOpenChange: vi.fn(), activeCount: 2, onClear },
      createElement('select', { 'aria-label': 'Person' }, createElement('option', null, 'All'))))
  ...
  it('uses accessible disclosure state and closes on Escape', () => {
    ...
    render(createElement(FilterDisclosure, { id: 'filters', open: true, onOpenChange, activeCount: 0, onClear: vi.fn() }, createElement('input')))
